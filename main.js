...
// инпут + label изменение клавиш (1 див)
  ...
  inputValues.setAttribute('placeholder', 'Ctr+q');
  ...
  // addLinkInput.setAttribute('value', 'onAddLink');
  // addLinkInput.setAttribute('checked', true);
  // addLinkInput.className='input_addLink';
  ...
  // saveDataInput.setAttribute('value', 'onSaveData');
  // saveDataInput.className='input_saveData';
...
document.addEventListener('keydown', function(event){
  if (event.ctrlKey && event.code == 'KeyQ' && event.key == 'q'){
...
function areaSelection(){
  ...
  function koordinats() {
    ...
    tags.forEach(tagEl => {
      ...
      if (!exclude) {
        ...
        let url = tagEl.getAttribute('href').replace('https://', '').replace('http://', '');
    ...
    result.forEach((resultEl, index) => {
      ...
      // чекбокс сохранять данные мд запусками 
  ...
  let inputCheckboxItem = document.querySelectorAll('.inputCheckboxItem').forEach((checkboxEl)=>{
    checkboxEl.addEventListener('click', ()=>{
      // console.log(checkboxEl)
    

      if(saveDataInput.checked){
        saveDataInput.value = 'on';
      }
      if(addLinkInput.checked){
        addLinkInput.value = 'on';
      }
      if(!saveDataInput.checked){
        saveDataInput.value = 'off';
      }
      if(!addLinkInput.checked) {
        addLinkInput.value = 'off';
      }
      localStorage.setItem('addLinkInputValue', addLinkInput.value);
      localStorage.setItem('saveDataInputValue', saveDataInput.value);
    ...
    //КНОПКА СОХРАНЕНИЯ****
      tabsBodySettingsBtn.addEventListener('click', ()=>{
        ...
        localStorage.getItem('addLinkInputValue');
        localStorage.getItem('saveDataInputValue');
        // console.log(localStorage.getItem('saveDataInputValue'));
        // console.log(localStorage.getItem('addLinkInputValue'));


       if (localStorage.getItem('saveDataInputValue') == 'on'){
        // saveDataInput.checked = true;
        saveDataInput.setAttribute('checked', true);
       }
       

        // if (addLinkInput.checked && addLinkInput.value == 'on'){
        //   addLinkInput.checked = true;
        // }
        // if (saveDataInput.checked && saveDataInput.value == 'on'){
        //   saveDataInput.checked = true;
        // }
        
        // if(!addLinkInput.checked){
        //   addLinkInput.checked = false;
        // }
        // if(!saveDataInput.checked){
        //   addLinkInput.checked = false;
        // }
...
// chrome.set({key: '123'}, function() {
//   console.log('Value is set to ');
// });

// chrome.get(['key'], function(result) {
//   console.log('result');
//   console.log(result);
